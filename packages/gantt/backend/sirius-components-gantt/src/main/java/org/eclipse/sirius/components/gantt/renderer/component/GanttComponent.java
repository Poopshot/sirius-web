...
public class GanttComponent implements IComponent {
    ...
    public Element render() {
        ...
        String targetObjectId = ganttDescription.getTargetObjectIdProvider().apply(variableManager);
        String label = optionalPreviousGantt.map(Gantt::getLabel).orElseGet(() -> ganttDescription.getLabelProvider().apply(variableManager));

        List<Task> previousTasks = optionalPreviousGantt.map(Gantt::tasks).orElse(List.of());

        Map<String, TaskDescription> id2TaskDescription = ganttDescription.getTaskDescriptions().stream()
                .collect(Collectors.toMap(TaskDescription::id, Function.identity()));
        List<Element> children = ganttDescription.getTaskDescriptions().stream()
                .map(taskDescription -> {
